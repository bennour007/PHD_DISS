...
Before we delve into the details of our approach and highlight why it improves mainstream relatedness measures we first showcase how the literature approach relatedness then we show how our approach takes the framework a step further.

### Relatedness as measured via co-occurence

In the REC literature relatedness is calculated based on the pair-wise observed frequency of co-occurrence. The idea is simple: frequent co-occurrences mean strong links between technologies. Here we show in detail how relatedness is measured, and the different normalization techniques used to harmonize the pairwise links.

To further elaborate, let's consider $c_{t,t'}$, the count of patents containing both technologies $t$ and $t'$ ($t\neq t'$) at a given year, such that $t, t' \in \mathcal{T}$, and $c_{t}$, $c_{t'}$ denote the total patent counts for each class individually.
...
**Association Strength** [@van2009software]

$$
\phi_{t,t'}^{\text{assoc}} = \frac{c_{t,t'} \cdot T}{c_t \cdot c_{t'}}
$$
where $T = \sum_{t \in \mathcal{T}} c_t$ is the total patent count. This probabilistic measure compares observed to expected co-occurrence under independence. Values above 1 indicate stronger-than-random association. It properly corrects for size effects, making it suitable for normalization purposes.

**Probability Index** [@steijn2017improvement]
...
$$
This improved probabilistic measure uses combinations without repetition, correcting for the fact that a patent cannot co-occur with itself. It provides more accurate estimates than association strength for patent co-occurrence analysis.

**Cosine Similarity** [@van2009software]
...
**Minimum Conditional Probability** [@hidalgo2007product]

$$
\phi_{t,t'}^{\text{min}} = \min\left\{P(\text{RTA}_{r,t} | \text{RTA}_{r,t'}), P(\text{RTA}_{r,t'} | \text{RTA}_{r,t})\right\}
$$

where $\text{RTA}_{r,t}$ indicates that region $r \in \mathcal{R}$ has a revealed technological advantage in technology $t$. This approach computes the minimum of pair-wise conditional probabilities of regions having RTA in one technology given they have RTA in another. 

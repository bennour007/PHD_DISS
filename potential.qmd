...
In the REC literature relatedness is calculated based on the pair-wise observed frequency of co-occurrences. The idea is simple: frequent co-occurrences mean strong links between technologies. Here we show in detail how relatedness is measured and outline the conceptual issues and the empirical consequences of the implicit underlying assumptions. 


As we briefly mentioned in the introduction, the relatedness framework is based on two constructs co-occurrence and linear aggregation of knowledge. These two constructs enable researchers to simplify the bi-partie relationships which is probably the reason of the framework's popularity. However, the simplification comes with the cost of lost information and potentially biased-or at best-misunderstood conclusions. In fact this was explicitly studied in the context of regional and income inequality, emissions, and most importantly industrial upgrading and path dependency. 

<PLEASE CITE>

However, path dependency is the most tangible issue in terms of policy in the context of the geography of innovation. This is also referred to as quiescence trap in @hidalgo2007. It implies that the initial condition of the geography dictates the level of success of related diversification effort. It's not hard to see why leveraging the advantages of regional technological landscape alone would not result in a successful innovative upgrading when the landscape itself is poorly diverse or clustered around a single activity. This has been explicitly modeled in different studies most interestingly in @antonietti2021going where the authors identified two approaches to diversification both accounting for the bi-partie nature identifying two distinct diversification trajectories that either prioritise place over technology or technology over place. 

The simplest measure of relatedness here would be to consider $c_{t,t'}$. However, as we mentioned in the introduction, co-occurrences are noisy and harmonizing the pairwise values is crucial. To this end, the REC literature considers different normalization methods which we will briefly outline later in this section.

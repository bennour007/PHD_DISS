...
### Contrast with Relatedness Density

Technological Potential differs from relatedness density in ways that directly address the limitations identified earlier:

| Limitation | Relatedness Density | Technological Potential |
|------------|--------------------|-----------------------|
| Dimensional mismatch | Estimates pairwise correlations from sparse co-occurrences | Learns patterns from pooled region-year observations |
| Symmetry | $\phi_{ij} = \phi_{ji}$ by construction | Separate models yield directional predictions |
| Linear aggregation | $\omega_{ri} = \sum_j M_{rj}\phi_{ij} / \sum_j \phi_{ij}$ | Tree structure captures interactions |
| Context-free | Same density for all regions with identical portfolios | Region-specific probability estimates |

The shift from correlation to prediction reframes what the measure captures. Density asks: "How similar is technology $i$ to what region $r$ already has?" Potential asks: "Given everything we know about how regions diversify, how likely is region $r$ to develop technology $i$?" The latter question—the one relevant for policy—requires the predictive machinery to answer.
...
### Illustrative Comparison: Relatedness versus FITS
...
The measures developed in this section address specific limitations of conventional relatedness approaches:

| Limitation | Conventional Approach | Our Response |
|------------|----------------------|--------------|
| Noise from dimensional mismatch | Pairwise co-occurrence correlations | Ensemble learning from pooled observations |
| Symmetry obscures directionality | $\phi_{ij} = \phi_{ji}$ | FITS extracts asymmetric dependencies |
| Linear aggregation discards interactions | Density as weighted sum | Tree structure captures conditionality |
| Context-free measures | Universal technology-pair scores | Region-specific Potential estimates |

...
In @tbl-potential-fe-ols-h (and later on in @tbl-potential-re) we extend our framework by incorporating covariates on the national entrepreneurial ecosystem characteristics from the Global Entrepreneurship Index. The idea here is to understand whether these covariates contribute to the variation in regional potential along side the network metrics (and their interactions). However, this specification for the national covariates serves as a robustness check for the models in @tbl-potential-re and tbl-potential-fe-ols, model 2 remains our preferred setting for these covariates. 
...
## Hierarchical structures and spatial implications
...
The sign flip occurs at approximately $-0.67$ SD: regions below this coherence threshold experience a negative portfolio-level betweenness effect, while regions above it experience a positive one. The interaction coefficient ($+0.0208$ total) is $142\%$ of the betweenness main effect ($+0.0140$). In a non-causal sense, coherence is the governing condition that determines the direction and magnitude here. A portfolio with bridge like technologies without adequate industrial alignment is not sufficient for regions to increase their aggregate potential. To embed and activate those bridging connections into portfolio-level feasibility advantage a baseline of aggregate industrial alignment is required. This is further illustrated in @eg_avg_coh, where we can see the association clearly for two groups of regions: those above the coherence threshold: Lombardia (ITC4, IT), Hannover (DE92, DE), and Provence-Alpes-Côte d'Azur (FRL0, FR); and those below it: Moravskoslezsko (CZ08, CZ), Zeeland (NL34, NL), and Cantabria (ES13, ES). In this table we can see that even with low average betweenness (third, forth, and seventh deciles), regions above the coherence threshold have higher average regional potential (ninth decile), whereas for the regions below the coherence threshold (having average coherence below the second decile) have lower average potential despite having higher average betweenness than the regions above the same threshold. This confirms and reflects known geographic dichotomies in Europe specifically the core-periphery divide. 



```{r eg_avg_coh}
#| tbl-cap: "Heterogeneity of average betweenness at different average coherence deciles"

tblavg <- readr::read_csv(here::here("resources/eg_avg_coh.csv"))

tblavg |> 
  dplyr::select(-year, - ctr) |>
  dplyr::mutate(coht = paste(coht, "coherence threshold")) |>
  dplyr::rename("decile pot" = potq, "decile coh" = cohq, "decile bet" = betq, "coherence threshold" = coht) |>
  gt::gt(groupname_col = "coherence threshold", rowname_col = "region") |> 
  gt::fmt_number(columns = c("reg_potential", "avg_coherence", "avg_betweenness"), decimals = 2) |>
  gt::fmt_number(columns = "avg_betweenness", decimal = 4)
